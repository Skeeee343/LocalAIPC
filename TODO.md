...
- [ ] 3. Super Productivity in playbook — decided: regular desktop install on daily-use machine + adapter beside it, Tailscale bridge to server (ADR-001 topology, runbook section written, `tailscale_authkey` wired). No SP container.
  > Needs your input: (a) which machine runs SP daily — this Mac or the Ubuntu box (desktop session?)? (b) install the SP app + enable its local REST API, paste `sp_token` to `secrets.yml`; (c) generate a reusable Tailscale pre-auth key (tailscale.com/admin) to `secrets.yml`. Then #6 connects them. If the adapter ends up on the Mac (outside Ansible), the server `mcp-adapter` compose service gets dropped at first run — flagged, not done.
